...
struct VersionVectorSerializationTests {
    ...
    func test_serializationOf_VersionVector() throws {
        ...
        let node = self.testCase.system.cluster.node
        deserialized.state.count.shouldEqual(2)  // replicas A and B
        "\(deserialized)".shouldContain("actor:sact://VersionVectorSerializationTests@127.0.0.1:\(node.endpoint.port)/user/A: 2")
        "\(deserialized)".shouldContain("actor:sact://VersionVectorSerializationTests@127.0.0.1:\(node.endpoint.port)/user/B: 5")
    }
    ...
    func test_serializationOf_VersionDot() throws {
        ...
        let node = self.testCase.system.cluster.node
        "\(deserialized)".shouldContain("actor:sact://VersionVectorSerializationTests@127.0.0.1:\(node.endpoint.port)/user/A")
