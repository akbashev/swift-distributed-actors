...
struct ClusterAssociationTests {
    ...
    func test_handshake_shouldStopTryingWhenMaxAttemptsExceeded() async throws {
        ...
        try logs.awaitLogContaining(self.testCase.testKit(first), text: "Giving up on handshake with node [sact://second@127.0.0.1:\(secondPort)]")
    }
    ...
    func test_handshake_shouldNotifyOnRejection() async throws {
        ...
        try self.testCase.capturedLogs(of: first)
            .awaitLogContaining(
                ...
                text: "Handshake rejected by [sact://second@127.0.0.1:\(second.cluster.node.endpoint.port)], reason: incompatibleProtocolVersion"
            )
    }
    ...
    func test_handshake_shouldRejectIfNodeIsLeavingOrDown() async throws {
        ...
        try self.testCase.capturedLogs(of: second)
            .awaitLogContaining(
                ...
                text: "Handshake rejected by [sact://first@127.0.0.1:\(first.cluster.node.endpoint.port)], reason: Node already leaving cluster."
            )
    }
