...
//    }
//
//    let mockPeers: Mutex<[_AddressableActorRef]> = Mutex([])
//    let logics: Mutex<[MembershipGossipLogic]> = Mutex([])
//
//    func logic(_ id: String) -> MembershipGossipLogic {
//        guard let logic = (self.logics.withLock { $0 }.first { $0.localNode.endpoint.systemName == id }) else {
...
//        self.logics.withLock { $0 }.map(\.latestGossip)
//    }
//
//    private func makeLogic(_ system: ClusterSystem, _ probe: ActorTestProbe<Cluster.MembershipGossip>, with testCase: ClusteredActorSystemsTestCase) -> MembershipGossipLogic {
//        MembershipGossipLogic(
//            GossipLogicContext<Cluster.MembershipGossip, Cluster.MembershipGossip>(
//                ownerContext: self.testCase.testKit(system).makeFakeContext(),
...
//        let systemA = await self.testCase.setUpNode("A") { settings in
...
//            setUpPeers: { () in
//                [
//                    Cluster.MembershipGossip.parse(initialGossipState, owner: systemA.cluster.node, nodes: self.testCase.nodes),
//                    Cluster.MembershipGossip.parse(initialGossipState, owner: systemB.cluster.node, nodes: self.testCase.nodes),
//                    Cluster.MembershipGossip.parse(initialGossipState, owner: systemC.cluster.node, nodes: self.testCase.nodes),
//                ]
//            },
//            updateLogic: { _ in
...
//        let systemB = await self.testCase.setUpNode("B")
//        let systemC = await self.testCase.setUpNode("C")
//        let systemD = await self.testCase.setUpNode("D")
//        let systemE = await self.testCase.setUpNode("E")
//        let systemF = await self.testCase.setUpNode("F")
//        let systemG = await self.testCase.setUpNode("G")
...
//                    Cluster.MembershipGossip.parse(initialNewGossip, owner: systemG.cluster.node, nodes: self.testCase.nodes),
//                    Cluster.MembershipGossip.parse(initialNewGossip, owner: systemH.cluster.node, nodes: self.testCase.nodes),
//                    Cluster.MembershipGossip.parse(initialNewGossip, owner: systemI.cluster.node, nodes: self.testCase.nodes),
//                    Cluster.MembershipGossip.parse(initialNewGossip, owner: systemJ.cluster.node, nodes: self.testCase.nodes),
//                ]
//            },
//            updateLogic: { _ in
//                let logicA: MembershipGossipLogic = self.logic("A")
...
//                """,
//                owner: systemD.cluster.node, nodes: self.testCase.nodes
//                ))
//            },
//            stopRunWhen: { (logics, _) in
...
//        for testCase: ClusteredActorSystemsTestCase,
//        runs: Int,
...
//        self.mockPeers.withLock {
//            $0 = try! self.testCase.systems.map { system -> _ActorRef<GossipShell<Cluster.MembershipGossip, Cluster.MembershipGossip>.Message> in
...
//                return self.testCase.systems.first!._resolveKnownRemote(ref, onRemoteSystem: system)
//            }.map(\.asAddressable)
//        }
//
//        var log = self.testCase.systems.first!.log
//        log[metadataKey: "actor/path"] = "/user/peer" // mock actor path for log capture
//
//        for _ in 1 ... runs {
//            // initialize with user provided gossips
//            self.logics.withLock {
//                $0 = initialGossips.map { initialGossip in
//                    let system = self.testCase.system(initialGossip.owner.systemName)
//                    let probe = self.testCase.testKit(system).makeTestProbe(expecting: Cluster.MembershipGossip.self)
//                    let logic = self.makeLogic(system, probe, with: testCase)
...
//                // information.
//                let participatingGossips = self.logics.withLock { $0.shuffled() }
...
//            }
//
//            updateLogic(self.logics.withLock { $0 })
//
//            var rounds = 0
//            log.notice("~~~~~~~~~~~~ new gossip instance ~~~~~~~~~~~~")
//            while !stopRunWhen(self.logics.withLock { $0 }, results) {
//                rounds += 1
...
//        Array(self.mockPeers.withLock { $0 .filter { $0.id.node != logic.localNode }})
//    }
//
//    func selectLogic(_ peer: _AddressableActorRef) -> MembershipGossipLogic {
//        (self.logics.withLock { $0.first { $0.localNode == peer.id.node }})!
//    }
//
//    func peer(_ logic: MembershipGossipLogic) -> _AddressableActorRef {
//        let nodeName = logic.localNode.endpoint.systemName
//        if let peer = (self.mockPeers.withLock { $0 }.first { $0.id.node.endpoint.systemName == nodeName }) {
//            return peer
//        } else {
